...
void WeaponComponent::Shoot(const glm::vec2& mousePosition)
{
	if (_reload_time <= 0.f)
	{
		...
		{
			std::shared_ptr<Game::PistolBullet> newBullet = GameManager::SpawnActor<Game::PistolBullet>("mush1", _sprite_weapon->GetPosition(), _sprite_weapon->GetSize() / 2.f, _sprite_weapon->GetRotation());
