...
void WeaponComponent::Shoot(const glm::vec2 mousePosition)
{
	...
	if (_reload_time <= 0.f)
	{
		...
		{
			std::shared_ptr<Game::PistolBullet> newBullet = GetWorld()->SpawnActor<Game::PistolBullet>("pistolBullet", _sprite_weapon->GetPosition(),
 _sprite_weapon->GetSize(), _sprite_weapon->GetRotation());
